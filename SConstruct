### ------------------------------------------------------------------
###  SConstruct - Building script for paolina
###
###  Author : 
###  Created: 
###  Version: 
###  paolina = Prototype Analisis Objects Library for Interactive Next Analysis
###  N.B. First time this script is executed, a configure-like step
###  tries to find out where are located the header and libraries
###  of paolina dependencies (so far, only ROOT). If it succeeds, 
...
def write_config(dir_prefix):
    ...
    s = s + '\n' + '\n'
    s = s + 'usage()' + '\n' + '{' + '\n' + '    cat  <<EOF' + '\n'
    s = s + 'Usage: paolina-config [OPTION]' + '\n' + '\n'
...
if not env['LIBPATH']: 
   ...
   ## Create a Configure object that provides autoconf-like functionality
    ...
    try:
        env.ParseConfig('root-config --cflags --glibs --evelibs')
    except OSError:
...
idir_inc    = env['PREFIX'] + '/include/paolina'
...
env.Rootcint('paolinaDict.cxx',headers+['src/LinkDef.h'])

## Create the library
sourcefiles = Glob('src/*.cc')  
libpaolina = env.SharedLibrary(idir_lib + '/paolina', ['paolinaDict.cxx']+sourcefiles)
...
env.Execute(Chmod(w_prefix_dir+'/bin/paolina-config', 0755))
...
Clean(libpaolina, CONF_FILE)
Clean(libpaolina, idir_bin+'/paolina-config')
